...
log_directory = "/mydata/Mininet-wifi-RL/output_multi_cluster"
dataset_directory = "/mydata/data_cluster"
...
def build_observation_matrix(t):
    ...
    for i in range(f):
        file_path = f"/mydata/Mininet-wifi-RL/output_multi_cluster/ch_received_from_sensor_cluster_0_sensor_{i}.txt"
        if os.path.exists(file_path):
            with open(file_path, 'r') as file:
                lines = file.readlines()
